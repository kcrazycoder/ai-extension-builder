...
describe('StorageService', () => {
  ...
  describe('uploadZip', () => {
    it('should upload ZIP file with correct metadata', async () => {
      ...
      expect(mockBucket.put).toHaveBeenCalledWith('test-key.zip', zipData, {
        httpMetadata: {
  ...
  describe('listZips', () => {
    it('should list ZIPs without prefix', async () => {
      const mockObjects = {
        objects: [{ key: 'file1.zip' }, { key: 'file2.zip' }],
      };
    ...
    it('should list ZIPs with prefix', async () => {
      const mockObjects = {
        objects: [{ key: 'user-123/file1.zip' }, { key: 'user-123/file2.zip' }],
      };
      mockBucket.list.mockResolvedValue(mockObjects);

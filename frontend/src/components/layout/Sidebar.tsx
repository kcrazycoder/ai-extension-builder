    ...
    return (
        <div className={cn(
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                ...
                {/* Top: History */}
                <div className="flex-1 overflow-y-auto px-2 space-y-1 scrollbar-hide min-h-0 pb-2">
                    {!isCollapsed && (
                        <div className="px-2 py-1.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider sticky top-0 bg-slate-50 dark:bg-zinc-950 z-10">
                            History
                        </div>
                    )}
                    {history.length === 0 ? (
                ...
                <div className="flex-1 overflow-y-auto px-2 space-y-1 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-zinc-800 min-h-0 border-t border-slate-200 dark:border-zinc-800 pt-2">
                    {!isCollapsed && (
                        <div className="px-2 py-1.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider sticky top-0 bg-slate-50 dark:bg-zinc-950 z-10">
                            My Extensions
                        </div>
                    ...
                    {history.map((ext) => (
                        <button
                            ...
                            onClick={() => onSelectExtension(ext)}
                            className={cn(
                                "w-full text-left p-3 rounded-lg flex items-start gap-3 transition-all group",
                                currentExtensionId === ext.id
                            ...
                            {!isCollapsed && (
                                <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                                    ...
                                    <div className="flex items-center justify-between gap-2 w-full">
                                        <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate" title={ext.name || ext.prompt}>
                                            {ext.name || ext.prompt}
                                        </span>
                                        <span className="flex-shrink-0 text-[10px] font-mono text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded border border-slate-200 dark:border-zinc-700">
                                            v{ext.version || '0.1.0'}
                                        </span>
                                    </div>
                                    {/* Row 2: Description */}
